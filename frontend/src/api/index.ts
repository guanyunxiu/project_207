export { default as request } from './request'
export { authApi } from './auth'
export { categoriesApi } from './categories'
export { documentsApi } from './documents'
export { filesApi } from './files'
export { usersApi } from './users'
export { notificationsApi } from './notifications'
export { assessmentsApi } from './assessments'
export { statisticsApi } from './statistics'
export { counselingApi } from './counseling'

export { authApi as auth } from './auth'
export { categoriesApi as categoryApi } from './categories'
export { documentsApi as documentApi } from './documents'
export { filesApi as fileApi } from './files'
export { usersApi as userApi } from './users'
export { notificationsApi as notificationApi } from './notifications'
export { assessmentsApi as assessmentApi } from './assessments'
export { statisticsApi as statsApi } from './statistics'
export { counselingApi as counselApi } from './counseling'
